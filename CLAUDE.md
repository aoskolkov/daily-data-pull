...
Dataflows (key order): `BOP` and `IIP` = `COUNTRY.BOP_ACCOUNTING_ENTRY.INDICATOR.UNIT.FREQUENCY`; `PIP` (**CPIS renamed**) = `COUNTRY.ACCOUNTING_ENTRY.INDICATOR.SECTOR.COUNTERPART_SECTOR.COUNTERPART_COUNTRY.FREQUENCY`; `DIP` (**CDIS renamed**) = `COUNTRY.DV_TYPE.INDICATOR.COUNTERPART_COUNTRY.FREQUENCY`; `ITG` (trade in goods totals) = `COUNTRY.INDICATOR.TYPE_OF_TRANSFORMATION.FREQUENCY`; `IMTS` (bilateral goods, the former DOTS) = `COUNTRY.INDICATOR.COUNTERPART_COUNTRY.FREQUENCY`. 222 dataflows in total (IL, MFS_*, CPI, ER, GFS_*, QNEA, WEO under agency `IMF.RES`, …); list them at `https://api.imf.org/external/sdmx/3.0/structure/dataflow`.

Gotchas:
- **Codes are split across two dimensions.** BOP FDI assets is `A_NFA_T` (accounting entry) × `D_F` (indicator); net is `NNAFANIL_T`; reserves use `A_T`; balances (`CAB`, `KAB`, `EO`) and current-account detail (`G`, `S`, `GS`, `IN1`, `IN2`, services `SC`/`SD`/`SG`/`SH`/`SI`/`SJ`) use `NETCD_T` for net, `CD_T` for credits (exports/receipts) and `DB_T` for debits. ITG needs `INDICATOR` × `TYPE_OF_TRANSFORMATION`: `XG.FOB_USD` (exports, 191 economies) and `MG.CIF_USD` (imports, 188); `MG.FOB_USD` exists for only ~11. IIP positions use `A_P`/`L_P`/`NETAL_P` with indicators `D`, `P_MV`, `P_F5_MV`, `P_F3_MV`, `O`, `R`, `IIP`, `NIIP`. Portfolio equity/debt have no published net.
...
- **Trade**: ITG is small (annual ~22k rows, monthly ~47k, seconds). IMTS is not: ~1.5M rows and ~5 min *per indicator* annually, so `chunk_by: INDICATOR`; quarterly/monthly are far larger. IMF services trade (`ITS`, agency `IMF.RES`) holds only derived measures and stops in 2015 — use the BOP services credits/debits instead. Exports are FOB and imports CIF, so `goods_balance_*` in `macrodata/trade/` is not the BPM6 balance; `imf_bop` `goods_net` is.
